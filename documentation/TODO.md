...
- [ ] **Every step rewrites every map on the scene.** A scene flag cannot be updated in part, so each completed move serializes the whole container -- all of that scene's maps, for every character -- and broadcasts it. With a full party on a well-explored scene that is megabytes per step, and it grows as the session goes on. Loading is now lazy and this is the matching gap on the writing side. Fixing it needs somewhere a single map can be written on its own, which a scene flag is not.
