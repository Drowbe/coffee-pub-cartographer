...
### i have lterally 45 minutes until ga
...
- **Mapping indicator**: A spinner beside the recording dot says the map is still being drawn. The marker arrives at the stop square with the token, but the linework for the route it took arrives a round trip later, so an area that has not filled in yet reads as pending rather than as a square the map missed.
...
- **Marker position while recording**: The party marker is where the token is. It previously sat on the last square the map had reached, on the reasoning that it should not claim more than the map knew — but that put it a full round trip behind the token and made it appear to jump when the map caught up, which read as a bug rather than as honesty. Position is now simply true, and how far the drawing has got is said by the chrome instead: the marker fades out while the token is moving, since the square it will stop on is genuinely unknown, and fades back in where it came to rest.
