...
- **Settled movement**: Dragging a token reliably lands the mapped position on the square where it stopped. A single catch-up pass could report progress that was already stale while a long drag was still drawing; the mapper now waits for its queue to drain and re-checks until the mapped square and the token square agree.
