...
- **Floor-side wall snapping**: Visible Foundry wall segments now snap to the nearest edge of an explored floor square on the token's side. Curved walls become continuous old-school stair-step boundaries instead of disconnected centerline hooks, and doors/windows use the same boundary vocabulary.
