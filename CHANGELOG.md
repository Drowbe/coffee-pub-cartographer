...
- **A map now belongs to somebody, and there are three kinds of somebody**: A map's identity used to *be* an Actor — it was filed under one, permissioned through one, and named after one — which left no way to express a map that is the party's or a map that is an artifact. A record now carries what owns it. A **player map** is a character's own, recorded as their token walks. The **party map** is held in common, one to a scene. An **official map** is an artifact: a plan of a place rather than a record of a visit. Everything that used to ask "does this user own the Actor?" now asks the question appropriate to the kind, and a record written before any of this reads as a player map — which is what it was — so nothing needs migrating.
- **Player maps are private until shared**: A character's map is now theirs alone, with an eye on its row to hand it to the rest of the party. Worth being plain about what that means: the record still travels to every client inside the scene flag, exactly as the walls themselves always have, so this decides whose list a map appears in rather than what a determined player could reach with a console open. For a table of people playing a game together that is the useful meaning of private, and the honest one to claim.
- **The party map, and two ways to fill it**: One shared map per scene, which any member may edit and only the GM may delete. It is filled by **donating** a map already recorded, or by pressing **Record** while looking at it — which gives the party everything as it is walked. The second is the first done continuously, and neither diverts the recording: the character always gets their own map of everywhere they went, because it is one reveal writing to both. Contributing is a decision, never implied by exploring; a player may keep their own map to themselves for their own reasons. The merge only ever adds, so donating twice does nothing the second time, and a square a character struck off their own map is *not* struck off the party's — somebody else may have walked it and given it to them, and their exploration is not the donor's to erase.
- **Official maps, authored rather than explored**: A GM names one and the whole scene arrives already mapped, as though a token had walked every part of it — no token involved, because a token was only ever a proxy for somebody having been somewhere and an artifact claims nothing of the sort. What counts as *inside* is settled by flooding inward from the edge of the scene: whatever the open ground cannot reach is enclosed, which needs no starting square to be guessed at, and doors seal the flood as walls do or it pours in through the front door. A scene whose rooms are not sealed has no inside to find and the map arrives blank, which is said out loud rather than left looking like a fault. From there the existing **Fix Things** and **Floor Type** menus are the drawing tools.
- **An artifact is hidden until it is found**: A plan of a dungeon is the sort of thing a party is meant to turn up, so an official map starts unseen and the GM reveals it when they have. Once revealed the party may write in its margins but not rub out what its author drew — each person may take back their own additions and nothing else, which is what makes it an artifact rather than another map to edit.
- **How heavily the squares are ruled**: A **Grid** submenu on every square offers Off, Light, Medium and Dark. It is a reading preference rather than anything about the map, so it is kept per person, sits outside every ownership check, and a player may turn the ruling down on a map they do not own without changing anybody else's view. Medium is exactly what every theme was drawn at before this was adjustable.
- **Plain floor shades**: Three flat washes with no pattern at all, alongside the seven materials. What a plain old-school map often wants is one room told apart from the next by tone rather than by what its floor is made of — and being pattern-free they are the cheapest thing the renderer draws.
- **Clearing a whole area of floor**: **Fix Things** gains *This whole area is not floor* beside the single square. The two answer different complaints and the wrong one is destructive: striking one square is how a misread corner gets fixed, and flooding from there would take the entire room that corner opens onto. The area version is bounded exactly as surfacing a floor is bounded, so it stops at the walls and doorways the map already knows about.
- **A note can hold a paragraph**: The note field was a single line of 240 characters, which made anything past a label feel like a misuse of the box it was typed into. It is now eight lines of a thousand, and the line breaks inside are kept — a room is worth a few sentences. Still only text, which is the whole convention: these are maps drawn in pencil, not documents.

### CHANGED

- **Floor patterns are quieter, and drawn at map scale**: A surface was heavier than the grid line it sat under, so the eye read it as linework rather than as texture; it is now roughly half the weight, below the ruling, where it belongs at the bottom of the hierarchy. Dirt, grass and rock were thinned to about a third of their density — at 36 pixels a square the old spacing read as noise rather than ground. Cobbles were an offset lattice of identical ellipses, which is a second grid laid over the map's own and exactly what a cobbled floor does not look like; they are now assorted sizes, each turned a different way. Boulders are angular outlines rather than circles, three to a tile instead of seven and half again as large, because a circle reads as a ball and a boulder is a lump. Brick, board and flagstone keep their lattices — that is what makes them read as brick, board and flagstone — but at half again the period and a lighter line, so they sit under the linework instead of arguing with the grid.
- **A map is named after its scene**: Titles carried the character, the party or the word *Official* as well, which said the same thing three times over — the row already names who mapped it and the heading above it says so again. Titles generated under the old scheme are cleaned up as they are read, matched exactly, so a name somebody chose for themselves survives untouched.
- **The Recorded Maps list follows the tool windows**: Maps are gathered into sections with a heading and a count and drawn as rows with a thumbnail, a title and its actions — the shape the other tool windows in the suite use. It is described entirely in the tool window's own palette, which is restated per theme, so it is correct in Light, Dark and Glass without this module deciding any of it. Within a section the shared maps lead: the artifact of the place, then the map the party holds together, then everybody's own. Recency is the right order among personal maps and the wrong one for those two, which are the same maps every time the list is opened and should not wander about in it.
- **Thumbnails show a corner of the map rather than all of it**: Fitting a whole dungeon into a 96-pixel tile put several squares in each pixel, so what survived was their average and every map came back the same grey blob. Squares are now held to a size that still reads as a square once the tile is scaled down, and the thumbnail shows as much as fits — a room and its corridors. It is framed on where the squares actually are rather than the middle of their bounding box, since one long corridor off to a side drags that middle into empty ground and the tile comes back blank. Small maps still fit whole, exactly as before.
- **Fitting the map is always offered**: It used to trade places with centring on the party, so the moment recording began the one control that recovers a lost view disappeared. Every map can be fitted; centring is the one that can be meaningless, since an artifact has nobody standing on it, so that is the one that now comes and goes.

### FIXED

- **Choosing any floor but Default made mapping crawl**: Every surfaced square became its own masked layer — thousands of them, re-rastered on every zoom step, each phased to its own position so no two could share a raster. The rock hatching had always done the identical per-square phasing with an ordinary background and never crawled, which is what identified the cause: the layer count, not the pattern. A surface is now one layer for the whole map, tiled once and cut to the squares carrying it, so at most one layer per material exists however large the map grows. Surfaces are also genuinely continuous now rather than thousands of separately aligned tiles talked into meeting at their seams — and pattern detail stopped being a performance question at all, which is why the patterns could then be redrawn freely.
- **A surface could cross a doorway**: A square that a curve cuts through skipped the boundary test entirely, because the curve cutting it *is* the wall it lies against and asking would have stopped it ever taking a surface. But a doorway is not that wall — it is a boundary the party recorded — so a surface could cross one whenever that doorway's square happened to be cut by a curve as well, and then carry on into the next room. Such a square is now asked half the question rather than none of it.
- **Clearing a floor could leave part of it behind**: Taking a surface away was bounded by the walls as they stand now, so a scene whose walls have been edited since, or a square struck off by hand, could split what was one area in two and leave the half the click did not land in unreachable except a square at a time. Clearing now follows the material wherever it runs. Two adjacent rooms of the same surface do clear together — they read as one expanse of stone, so clearing them as one is the honest reading of the click.
- **Anyone could mark up any map**: Placing and removing symbols are open to the whole party by design, and that exemption skipped every check rather than only the ownership one — so the rule was enforced by the client that asked and nothing else. A map somebody cannot see is not a map they may write on, which stopped being a theoretical distinction the moment artifacts began arriving hidden.
- **Asking whether a square is on the map no longer scans the whole map**: Six places answered it by walking an array that can hold a six-figure number of squares, once for every context menu opened and once for every change made. The answer is now worked out once per map and held against the record itself, so it cannot go stale: every write replaces the record whole, and a new one simply has no answer yet.
- **The window chrome could be built for the wrong view**: Rendering the body yields, and the toolbar was assembled afterwards from whatever view the window was in by then rather than the one the body had been drawn from — so anything switching views mid-render left the chrome reading a field that view never fills in. The chrome is now built from the same snapshot as the body, and a fresh render puts both right.
