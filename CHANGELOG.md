...
- **The map list is gathered**: A campaign grows one map per character per scene, so the Recorded Maps list became a wall of near-identical rows, every one reading *someone at somewhere*, with no way to pick out the wanted one. Maps now sit under headings — by scene, or by character, toggled from the top bar and remembered per player — and each row only has to name whichever of the two its heading does not. The scene being stood in, or the character selected, comes first; the rest follow by how recently they were drawn on.
